...
#   BACKUP_PATH           — prefix inside bucket (default: instacop)
...
if [ -n "${BACKUP_BUCKET:-}" ]; then
  aws s3 cp "/tmp/${DEST}" "s3://${BACKUP_BUCKET}/${BACKUP_PATH:-instacop}/${DEST}" \
    ${AWS_ENDPOINT_URL:+--endpoint-url "$AWS_ENDPOINT_URL"}
  # delete backups older than 14; keep the 14 most recent
  aws s3 ls "s3://${BACKUP_BUCKET}/${BACKUP_PATH:-instacop}/" \
    ${AWS_ENDPOINT_URL:+--endpoint-url "$AWS_ENDPOINT_URL"} | \
    awk '{print $4}' | sort | head -n -14 | while read -r old; do
      [ -n "$old" ] && aws s3 rm "s3://${BACKUP_BUCKET}/${BACKUP_PATH:-instacop}/$old" \
        ${AWS_ENDPOINT_URL:+--endpoint-url "$AWS_ENDPOINT_URL"}
    done
fi
